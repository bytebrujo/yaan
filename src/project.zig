...
pub fn addDeployFile(io: std.Io, allocator: std.mem.Allocator, target: []const u8, framework_url: []const u8, framework_version: []const u8) !void {
    ...
    if (std.mem.eql(u8, target, "docker")) {
    ...
    } else if (std.mem.eql(u8, target, "tencent")) {
        ...
        std.debug.print(
            ...
            \\    real-name authentication (实名认证) before any function can be created.
            \\
        , .{});
    } else if (std.mem.eql(u8, target, "alibaba")) {
        try writeDeployFileIfAbsent(io, cwd, "bootstrap", alibaba_bootstrap_template);
        ...
        std.debug.print(
            ...
            \\    so the credentials need FC + OSS permissions (see docs/deploy-alibaba.md).
            \\
        , .{});
    } else if (std.mem.eql(u8, target, "azure")) {
        try writeDeployFileIfAbsent(io, cwd, "bootstrap", azure_bootstrap_template);
        ...
        std.debug.print(
            ...
            \\    account are auto-created. `host.json` wires the custom handler + catch-all route.
            \\
        , .{});
    } else {
...
const tencent_scf_bootstrap_template =
    \\#!/bin/sh
    \\# Generated by `yaan add tencent`. Tencent SCF web functions proxy HTTP to a
    \\# server listening on 0.0.0.0:9000; this launches the Yaan single binary
    \\# there. TLS is terminated by SCF and forwarded as X-Forwarded-*, so
    \\# --trust-forwarded makes --force-https / secure cookies behave. Add flags
    \\# (--force-https, --hsts, --csrf, --max-body ...) as your app needs.
    \\exec ./yaan-app --host 0.0.0.0 --port 9000 --trust-forwarded
    \\
;

const tencent_deploy_sh_template =
    \\#!/usr/bin/env bash
    \\# Generated by `yaan add tencent`. Deploys this app to Tencent Cloud SCF as an
    \\# HTTP "web function": builds the static single binary, packages it with an
    \\# scf_bootstrap launcher into a zip, and creates/updates the function via tccli.
    \\# No container registry needed — the binary is uploaded directly (zip < 50MB).
    \\#
    ...
    \\MEMORY="${MEMORY:-256}"
    \\ROLE="${ROLE:-SCF_QcsRole}"
    \\TARGET="${TARGET:-x86_64-linux-musl}"
    \\# SCF web functions reject the "CustomRuntime" token, so we declare a standard
    \\# web runtime as a carrier and let scf_bootstrap drive startup (it can launch
    \\# any process). The static musl binary needs nothing from the base image.
    \\RUNTIME="${RUNTIME:-Go1}"
    ...
    \\
    \\BIN="zig-out/bin/yaan-app"
    \\if [ -z "$DRY_RUN" ] && [ ! -f "$BIN" ]; then
    \\  echo "error: $BIN not found after build (is b.installArtifact(app_server) in build.zig?)"; exit 1
    \\fi
    \\
    \\# 2. Stage the package: the binary + an executable scf_bootstrap at the zip root.
    \\STAGE="$(mktemp -d)"
    \\trap 'rm -rf "$STAGE"' EXIT
    \\if [ -z "$DRY_RUN" ]; then
    \\  cp "$BIN" "$STAGE/yaan-app"
    \\  if [ -f scf_bootstrap ]; then
    \\    cp scf_bootstrap "$STAGE/scf_bootstrap"
    \\  else
    \\    printf '#!/bin/sh\nexec ./yaan-app --host 0.0.0.0 --port 9000 --trust-forwarded\n' > "$STAGE/scf_bootstrap"
    \\  fi
    \\  chmod +x "$STAGE/yaan-app" "$STAGE/scf_bootstrap"
    \\  ( cd "$STAGE" && zip -j -q package.zip yaan-app scf_bootstrap )
    \\fi
    \\echo "+ zip -j package.zip yaan-app scf_bootstrap"
    \\
    \\# 3. Assemble the optional environment-variable JSON (K=V,K2=V2 -> SCF format).
    \\ENV_JSON=""
    ...
    \\fi
    \\
    \\# 4. Create the function if it does not exist yet, otherwise update its code.
    \\exists=""
    \\if tccli scf GetFunction --region "$REGION" --Namespace "$NAMESPACE" --FunctionName "$FUNCTION" >/dev/null 2>&1; then
    \\  exists=1
    \\fi
    \\
    \\if [ -z "$DRY_RUN" ]; then
    \\  ZIPB64="$(base64 < "$STAGE/package.zip" | tr -d '\n')"
    \\else
    \\  ZIPB64="<base64 of package.zip>"
    \\fi
    \\
    \\if [ -n "$exists" ]; then
    \\  echo "updating function $FUNCTION ..."
    \\  printf '{"FunctionName":"%s","Namespace":"%s","Handler":"scf_bootstrap","Code":{"ZipFile":"%s"}}' \
    \\    "$FUNCTION" "$NAMESPACE" "$ZIPB64" > "$STAGE/code.json"
    \\  run tccli scf UpdateFunctionCode --region "$REGION" --cli-input-json "file://$STAGE/code.json"
    \\  printf '{"FunctionName":"%s","Namespace":"%s","MemorySize":%s%s}' \
    \\    "$FUNCTION" "$NAMESPACE" "$MEMORY" "$ENV_JSON" > "$STAGE/cfg.json"
    \\  run tccli scf UpdateFunctionConfiguration --region "$REGION" --cli-input-json "file://$STAGE/cfg.json"
    \\else
    \\  echo "creating function $FUNCTION ..."
    \\  printf '{"FunctionName":"%s","Namespace":"%s","Type":"HTTP","Runtime":"%s","Handler":"scf_bootstrap","Role":"%s","Description":"Yaan app (SCF web function).","MemorySize":%s,"Timeout":60,"Code":{"ZipFile":"%s"},"PublicNetConfig":{"PublicNetStatus":"ENABLE","EipConfig":{"EipStatus":"DISABLE"}}%s}' \
    \\    "$FUNCTION" "$NAMESPACE" "$RUNTIME" "$ROLE" "$MEMORY" "$ZIPB64" "$ENV_JSON" > "$STAGE/create.json"
    \\  run tccli scf CreateFunction --region "$REGION" --cli-input-json "file://$STAGE/create.json"
    \\fi
    \\
    \\if [ -n "$DRY_RUN" ]; then echo "[dry-run] no changes made"; exit 0; fi
    \\
    \\# 5. Wait for the function to become Active, then print its public URL.
    \\printf 'waiting for %s to become Active' "$FUNCTION"
    \\for _ in $(seq 1 60); do
    \\  status="$(tccli scf GetFunction --region "$REGION" --Namespace "$NAMESPACE" --FunctionName "$FUNCTION" --filter Status 2>/dev/null || true)"
    \\  case "$status" in
    \\    Active) echo " ok"; break ;;
    \\    *Failed*) echo; echo "error: function entered status: $status"; exit 1 ;;
    \\    *) printf '.'; sleep 2 ;;
    \\  esac
    \\done
    \\
    \\host="$(tccli scf GetFunction --region "$REGION" --Namespace "$NAMESPACE" --FunctionName "$FUNCTION" --filter AccessInfo.Host 2>/dev/null || true)"
    \\if [ -n "$host" ]; then
    \\  echo "deployed: https://$host"
    \\else
    \\  echo "deployed. Enable public access to get a URL, or attach an API Gateway trigger."
    \\fi
    \\
;

const alibaba_bootstrap_template =
    \\#!/bin/bash
    \\# Generated by `yaan add alibaba`. Alibaba Function Compute custom-runtime
    \\# functions run an executable `bootstrap` and proxy HTTP to a server on
    \\# 0.0.0.0:9000; this launches the Yaan single binary there. FC terminates TLS
    \\# and forwards X-Forwarded-*, so --trust-forwarded makes --force-https /
    \\# secure cookies behave. Add flags (--force-https, --hsts, --csrf ...) as needed.
    \\exec ./yaan-app --host 0.0.0.0 --port 9000 --trust-forwarded
    \\
;

const alibaba_deploy_sh_template =
    \\#!/usr/bin/env bash
    \\# Generated by `yaan add alibaba`. Deploys this app to Alibaba Cloud Function
    \\# Compute (FC 3.0) as a custom-runtime HTTP function: builds the static single
    \\# binary, packages it with a `bootstrap` launcher, uploads the zip to OSS, and
    \\# creates/updates the function plus an anonymous HTTP trigger via the `aliyun`
    \\# CLI. FC reads code from OSS (the CLI can't inline a multi-MB zip past
    \\# ARG_MAX), so the deploy auto-creates an OSS bucket `yaan-fc-<accountid>`
    \\# (override with OSS_BUCKET).
    \\#
    \\# Override with env vars (all optional):
    ...
    \\CPU="${CPU:-0.35}"
    \\DISK="${DISK:-512}"
    \\# Custom runtime: FC runs an executable `bootstrap`; the static musl binary
    \\# needs nothing from the base image. Valid: custom / custom.debian10/11/12.
    ...
    \\fi
    \\
    \\# 2. Stage the package: the binary + an executable `bootstrap` at the zip root.
    \\STAGE="$(mktemp -d)"
    \\trap 'rm -rf "$STAGE"' EXIT
    \\if [ -z "$DRY_RUN" ]; then
    \\  cp "$BIN" "$STAGE/yaan-app"
    ...
    \\    printf '#!/bin/bash\nexec ./yaan-app --host 0.0.0.0 --port 9000 --trust-forwarded\n' > "$STAGE/bootstrap"
    \\  fi
    \\  chmod +x "$STAGE/yaan-app" "$STAGE/bootstrap"
    \\  ( cd "$STAGE" && zip -j -q package.zip yaan-app bootstrap )
    \\fi
    \\echo "+ zip -j package.zip yaan-app bootstrap"
    \\
    \\# 3. Upload the code zip to OSS (FC reads the code package from there).
    \\if [ -z "$DRY_RUN" ]; then
    \\  ACCOUNT_ID="$(aliyun sts GetCallerIdentity 2>/dev/null | grep -o '"AccountId"[^,]*' | grep -o '[0-9]\{6,\}' || true)"
    \\else
    \\  ACCOUNT_ID="ACCOUNTID"
    \\fi
    \\BUCKET="${OSS_BUCKET:-yaan-fc-${ACCOUNT_ID:-app}}"
    \\OBJECT="yaan/${FUNCTION}.zip"
    \\if [ -z "$DRY_RUN" ]; then
    \\  aliyun oss mb "oss://$BUCKET" --region "$REGION" >/dev/null 2>&1 || true
    \\  run aliyun oss cp "$STAGE/package.zip" "oss://$BUCKET/$OBJECT" --region "$REGION" --force
    \\else
    \\  echo "+ aliyun oss cp package.zip oss://$BUCKET/$OBJECT --region $REGION --force"
    \\fi
    \\
    \\# 4. Assemble the function config and create or update the function.
    \\ENV_JSON=""
    \\if [ -n "$SET_ENV_VARS" ]; then
    \\  pairs=""
    ...
    \\fi
    \\ROLE_JSON=""
    \\[ -n "$ROLE" ] && ROLE_JSON=",\"role\":\"$ROLE\""
    \\CODE_JSON="\"code\":{\"ossBucketName\":\"$BUCKET\",\"ossObjectName\":\"$OBJECT\"}"
    \\
    \\exists=""
    \\if [ -z "$DRY_RUN" ] && aliyun fc GET "/2023-03-30/functions/$FUNCTION" --region "$REGION" >/dev/null 2>&1; then
    \\  exists=1
    \\fi
    \\
    \\if [ -n "$exists" ]; then
    \\  echo "updating function $FUNCTION ..."
    \\  printf '{%s%s}' "$CODE_JSON" "$ENV_JSON" > "$STAGE/update.json"
    \\  run aliyun fc PUT "/2023-03-30/functions/$FUNCTION" --region "$REGION" --body "$(cat "$STAGE/update.json")"
    \\else
    \\  echo "creating function $FUNCTION ..."
    \\  printf '{"functionName":"%s","runtime":"%s","handler":"index.handler","cpu":%s,"memorySize":%s,"diskSize":%s,"timeout":60,"instanceConcurrency":20,"customRuntimeConfig":{"command":["./bootstrap"],"port":9000},%s%s%s}' \
    \\    "$FUNCTION" "$RUNTIME" "$CPU" "$MEMORY" "$DISK" "$CODE_JSON" "$ENV_JSON" "$ROLE_JSON" > "$STAGE/create.json"
    \\  run aliyun fc POST "/2023-03-30/functions" --region "$REGION" --body "$(cat "$STAGE/create.json")"
    \\fi
    \\
    \\if [ -n "$DRY_RUN" ]; then echo "[dry-run] no changes made"; exit 0; fi
    \\
    \\# 5. Ensure an anonymous public HTTP trigger, then print the function URL.
    \\cat > "$STAGE/trigger.json" <<'EOF'
    \\{"triggerName":"http","triggerType":"http","triggerConfig":"{\"authType\":\"anonymous\",\"methods\":[\"GET\",\"POST\",\"PUT\",\"DELETE\",\"HEAD\",\"OPTIONS\",\"PATCH\"],\"disableURLInternet\":false}"}
    \\EOF
    \\aliyun fc POST "/2023-03-30/functions/$FUNCTION/triggers" --region "$REGION" --body "$(cat "$STAGE/trigger.json")" >/dev/null 2>&1 || true
    \\
    \\URL="$(aliyun fc GET "/2023-03-30/functions/$FUNCTION/triggers/http" --region "$REGION" 2>/dev/null | grep -o '"urlInternet"[^,]*' | grep -o 'https[^"]*' | head -1 || true)"
    \\if [ -n "$URL" ]; then
    \\  echo "deployed: $URL"
    \\else
    \\  echo "deployed. Check the FC console for the function's HTTP trigger URL."
    \\fi
    \\
;

const azure_bootstrap_template =
    \\#!/bin/sh
    \\# Generated by `yaan add azure`. Azure Functions custom handlers proxy HTTP to
    \\# a server on the port in $FUNCTIONS_CUSTOMHANDLER_PORT; this launches the Yaan
    \\# single binary there. Azure terminates TLS and forwards X-Forwarded-*, so
    \\# --trust-forwarded makes --force-https / secure cookies behave. Add flags
    \\# (--force-https, --hsts, --csrf ...) as your app needs.
    \\exec ./yaan-app --host 0.0.0.0 --port "${FUNCTIONS_CUSTOMHANDLER_PORT:-8080}" --trust-forwarded
    \\
;

const azure_host_json_template =
    \\{
    \\  "version": "2.0",
    \\  "customHandler": {
    \\    "description": { "defaultExecutablePath": "bootstrap" },
...
const azure_deploy_sh_template =
    \\#!/usr/bin/env bash
    \\# Generated by `yaan add azure`. Deploys this app to Azure Functions as a custom
    \\# handler: builds the static single binary, packages it with a host.json +
    \\# bootstrap launcher + a catch-all HTTP function, and creates/updates the
    \\# function app via the `az` CLI. No container registry — the binary is
    \\# zip-deployed; Azure Functions just needs a resource group + storage account
    \\# (both auto-created).
    \\#
    ...
    \\RESOURCE_GROUP="${RESOURCE_GROUP:-yaan-rg}"
    \\TARGET="${TARGET:-x86_64-linux-musl}"
    \\SET_ENV_VARS="${SET_ENV_VARS:-}"
    \\DRY_RUN="${DRY_RUN:-}"
    \\
    ...
    \\run() { echo "+ $*"; if [ -z "$DRY_RUN" ]; then "$@"; fi; }
    \\
    \\# Function-app and storage-account names are globally unique; derive stable
    \\# defaults from the subscription id (override with FUNCTION / STORAGE_ACCOUNT).
    \\if [ -z "$DRY_RUN" ]; then
    \\  SUB="$(az account show --query id -o tsv 2>/dev/null | tr -d '-')"
    \\else
    \\  SUB="00000000000000000000000000000000"
    \\fi
    \\SHORT="$(printf '%s' "$SUB" | cut -c1-12)"
    \\FUNCTION="${FUNCTION:-yaan-$SHORT}"
    \\STORAGE_ACCOUNT="${STORAGE_ACCOUNT:-yaan$SHORT}"
    \\
    \\# 1. Build the static single binary (dist/ embedded; no toolchain at runtime).
    \\run zig build -Doptimize=ReleaseFast -Dtarget="$TARGET"
    \\BIN="zig-out/bin/yaan-app"
    ...
    \\fi
    \\
    \\# 2. Ensure the resource group, storage account, and function app exist.
    \\run az group create -n "$RESOURCE_GROUP" -l "$REGION" -o none
    \\if [ -n "$DRY_RUN" ] || ! az storage account show -n "$STORAGE_ACCOUNT" -g "$RESOURCE_GROUP" >/dev/null 2>&1; then
    \\  run az storage account create -n "$STORAGE_ACCOUNT" -g "$RESOURCE_GROUP" -l "$REGION" --sku Standard_LRS -o none
    \\fi
    \\if [ -n "$DRY_RUN" ] || ! az functionapp show -n "$FUNCTION" -g "$RESOURCE_GROUP" >/dev/null 2>&1; then
    \\  run az functionapp create -n "$FUNCTION" -g "$RESOURCE_GROUP" --storage-account "$STORAGE_ACCOUNT" \
    \\    --consumption-plan-location "$REGION" --os-type Linux --runtime custom --functions-version 4 -o none
    \\fi
    \\
    ...
    \\  fi
    \\  if [ -f host.json ]; then
    \\    cp host.json "$STAGE/host.json"
    \\  else
    \\    cat > "$STAGE/host.json" <<'HJ'
    \\{
    \\  "version": "2.0",
    \\  "customHandler": {
    \\    "description": { "defaultExecutablePath": "bootstrap" },
    ...
    \\HJ
    \\  fi
    \\  cat > "$STAGE/http/function.json" <<'FJ'
    \\{
    \\  "bindings": [
    \\    { "type": "httpTrigger", "direction": "in", "name": "req",
    \\      "methods": ["get","post","put","delete","patch","head","options"],
    \\      "authLevel": "anonymous", "route": "{*path}" },
    \\    { "type": "http", "direction": "out", "name": "res" }
    \\  ]
    \\}
    \\FJ
    \\  chmod +x "$STAGE/bootstrap" "$STAGE/yaan-app"
    \\  ( cd "$STAGE" && zip -r -q app.zip . )
    \\fi
    \\echo "+ zip app.zip host.json bootstrap yaan-app http/function.json"
    \\
    \\if [ -n "$DRY_RUN" ]; then echo "[dry-run] no changes made"; exit 0; fi
    \\
    \\# 5. Zip-deploy and print the URL.
    \\run az functionapp deployment source config-zip -g "$RESOURCE_GROUP" -n "$FUNCTION" --src "$STAGE/app.zip" -o none
    \\echo "deployed: https://$FUNCTION.azurewebsites.net"
    \\
;
...
pub const TencentScfDeploy = struct {
    /// SCF function name (also the default; the uploaded binary is always
    /// `yaan-app` regardless of this).
    function: []const u8,
    /// SCF region; null defers to the deploy script's default (ap-guangzhou).
    region: ?[]const u8,
    namespace: []const u8,
    /// Function memory in MB (string, passed through to the script).
    memory: ?[]const u8,
    /// CAM role the function assumes (first-time accounts need it to exist).
    role: []const u8,
    /// Passed to SCF as `Environment.Variables` (e.g. "K=V,K2=V2").
    set_env_vars: ?[]const u8,
    dry_run: bool,
};

/// Deploys the app to Tencent Cloud SCF as an HTTP "web function" by running the
/// `deploy.tencent.sh` pipeline (build the static binary → zip it with an
/// scf_bootstrap launcher → create/update via `tccli`). Prefers an on-disk
/// `deploy.tencent.sh` (so `yaan add tencent` users can customize it); otherwise
/// writes the built-in template to `.yaan/` and runs that. Options are passed to
/// the script as environment overrides via `env`, so the parent environment
/// (PATH for `tccli`/`zig`) is inherited unchanged. `dry_run` prints the steps
/// without building or mutating anything.
pub fn deployTencentScf(io: std.Io, allocator: std.mem.Allocator, opts: TencentScfDeploy) !void {
    const cwd = std.Io.Dir.cwd();
    const script_path: []const u8 = if (cwd.access(io, "deploy.tencent.sh", .{})) |_|
        "deploy.tencent.sh"
    else |_| blk: {
        cwd.createDirPath(io, ".yaan") catch {};
        try cwd.writeFile(io, .{ .sub_path = ".yaan/deploy.tencent.sh", .data = tencent_deploy_sh_template });
    ...
    try argv.append(allocator, try std.fmt.allocPrint(allocator, "FUNCTION={s}", .{opts.function}));
    try argv.append(allocator, try std.fmt.allocPrint(allocator, "NAMESPACE={s}", .{opts.namespace}));
    try argv.append(allocator, try std.fmt.allocPrint(allocator, "ROLE={s}", .{opts.role}));
    if (opts.region) |r| try argv.append(allocator, try std.fmt.allocPrint(allocator, "REGION={s}", .{r}));
...
pub const AlibabaFcDeploy = struct {
    /// FC function name (the uploaded binary is always `yaan-app`).
    function: []const u8,
    /// FC region; null defers to the deploy script's default (ap-southeast-1).
    region: ?[]const u8,
    /// Function memory in MB and CPU in vCPU (strings, passed through).
    memory: ?[]const u8,
    cpu: ?[]const u8,
    /// OSS bucket for the code zip; null lets the script derive `yaan-fc-<id>`.
    oss_bucket: ?[]const u8,
    /// RAM role ARN the function assumes (optional for a plain HTTP app).
    role: ?[]const u8,
    /// Passed to FC as `environmentVariables` (e.g. "K=V,K2=V2").
    set_env_vars: ?[]const u8,
    dry_run: bool,
};

/// Deploys the app to Alibaba Cloud Function Compute (FC 3.0) as a custom-runtime
/// HTTP function by running the `deploy.alibaba.sh` pipeline (build the static
/// binary → zip it with a `bootstrap` launcher → upload to OSS → create/update
/// the function + an anonymous HTTP trigger via `aliyun`). Like the Tencent path,
/// it prefers an on-disk `deploy.alibaba.sh` and otherwise writes the built-in
/// template to `.yaan/`; options are passed as environment overrides via `env`.
/// `dry_run` prints the steps without building or mutating anything.
pub fn deployAlibabaFc(io: std.Io, allocator: std.mem.Allocator, opts: AlibabaFcDeploy) !void {
    const cwd = std.Io.Dir.cwd();
    const script_path: []const u8 = if (cwd.access(io, "deploy.alibaba.sh", .{})) |_|
        "deploy.alibaba.sh"
    else |_| blk: {
        cwd.createDirPath(io, ".yaan") catch {};
        try cwd.writeFile(io, .{ .sub_path = ".yaan/deploy.alibaba.sh", .data = alibaba_deploy_sh_template });
        break :blk ".yaan/deploy.alibaba.sh";
    };
    ...
    if (opts.region) |r| try argv.append(allocator, try std.fmt.allocPrint(allocator, "REGION={s}", .{r}));
    if (opts.memory) |m| try argv.append(allocator, try std.fmt.allocPrint(allocator, "MEMORY={s}", .{m}));
    if (opts.cpu) |c| try argv.append(allocator, try std.fmt.allocPrint(allocator, "CPU={s}", .{c}));
    if (opts.oss_bucket) |b| try argv.append(allocator, try std.fmt.allocPrint(allocator, "OSS_BUCKET={s}", .{b}));
...
pub const AzureFunctionsDeploy = struct {
    /// Function app name (globally unique); null lets the script derive
    /// `yaan-<subid>`. The uploaded binary is always `yaan-app`.
    function: ?[]const u8,
    /// Azure region; null defers to the deploy script's default (eastus).
    region: ?[]const u8,
    resource_group: ?[]const u8,
    /// Storage account (Functions requires one); null lets the script derive it.
    storage_account: ?[]const u8,
    /// Set as function-app settings (e.g. "K=V,K2=V2"), readable as env vars.
    set_env_vars: ?[]const u8,
    dry_run: bool,
};

/// Deploys the app to Azure Functions as a custom handler by running the
/// `deploy.azure.sh` pipeline (build the static binary → ensure resource
/// group/storage/function app → zip host.json + bootstrap + binary + a catch-all
/// HTTP function → zip-deploy via `az`). Like the other serverless targets it
/// prefers an on-disk `deploy.azure.sh` and otherwise writes the built-in
/// template to `.yaan/`; options are passed as environment overrides via `env`.
/// `dry_run` prints the steps without building or mutating anything.
pub fn deployAzureFunctions(io: std.Io, allocator: std.mem.Allocator, opts: AzureFunctionsDeploy) !void {
    const cwd = std.Io.Dir.cwd();
    const script_path: []const u8 = if (cwd.access(io, "deploy.azure.sh", .{})) |_|
        "deploy.azure.sh"
    else |_| blk: {
        cwd.createDirPath(io, ".yaan") catch {};
        try cwd.writeFile(io, .{ .sub_path = ".yaan/deploy.azure.sh", .data = azure_deploy_sh_template });
        break :blk ".yaan/deploy.azure.sh";
    };

    var argv: std.ArrayList([]const u8) = .empty;
    defer argv.deinit(allocator);
    try argv.append(allocator, "env");
    if (opts.function) |f| try argv.append(allocator, try std.fmt.allocPrint(allocator, "FUNCTION={s}", .{f}));
    if (opts.region) |r| try argv.append(allocator, try std.fmt.allocPrint(allocator, "REGION={s}", .{r}));
    if (opts.resource_group) |g| try argv.append(allocator, try std.fmt.allocPrint(allocator, "RESOURCE_GROUP={s}", .{g}));
...
test "tencent deploy templates carry the SCF web-function invariants" {
    const contains = std.mem.containsAtLeast;
    // scf_bootstrap must launch the binary on the web-function port (9000) with
    // forwarded-header trust (SCF terminates TLS upstream).
    try std.testing.expect(contains(u8, tencent_scf_bootstrap_template, 1, "--host 0.0.0.0 --port 9000"));
    try std.testing.expect(contains(u8, tencent_scf_bootstrap_template, 1, "--trust-forwarded"));
    // The deploy script encodes the reverse-engineered, non-obvious recipe: an
    // HTTP web function with a standard runtime as the carrier (HTTP rejects the
    // "CustomRuntime" token), startup driven by scf_bootstrap, public access with
    // a nested EipConfig, and the zip uploaded via `--cli-input-json file://` to
    // sidestep the command-line length limit.
    inline for (.{
        "\"Type\":\"HTTP\"",
        "RUNTIME:-Go1}", // a standard web runtime as the carrier
        "\"Runtime\":\"%s\"",
        ...
        try std.testing.expect(contains(u8, tencent_deploy_sh_template, 1, needle));
    }
}

test "alibaba deploy templates carry the FC 3.0 custom-runtime invariants" {
    const contains = std.mem.containsAtLeast;
    // bootstrap launches the binary on FC's custom-runtime port (9000).
    try std.testing.expect(contains(u8, alibaba_bootstrap_template, 1, "--host 0.0.0.0 --port 9000"));
    try std.testing.expect(contains(u8, alibaba_bootstrap_template, 1, "--trust-forwarded"));
    // The deploy script encodes the FC 3.0 recipe (validated against the FC
    // OpenAPI metadata): a custom.debian10 function whose customRuntimeConfig runs
    // ./bootstrap on port 9000, code uploaded to OSS (the CLI can't inline a
    // multi-MB zip past ARG_MAX) and referenced by ossBucketName/ossObjectName,
    // plus an anonymous HTTP trigger for the public URL.
    inline for (.{
        "\"runtime\":\"%s\"",
        ...
        try std.testing.expect(contains(u8, alibaba_deploy_sh_template, 1, needle));
    }
}

test "azure deploy templates carry the Functions custom-handler invariants" {
    const contains = std.mem.containsAtLeast;
    // bootstrap maps Azure's custom-handler port to the server (validated live).
    try std.testing.expect(contains(u8, azure_bootstrap_template, 1, "FUNCTIONS_CUSTOMHANDLER_PORT"));
    try std.testing.expect(contains(u8, azure_bootstrap_template, 1, "--host 0.0.0.0 --port"));
    // host.json wires the custom handler with raw HTTP forwarding, served at root.
    try std.testing.expect(contains(u8, azure_host_json_template, 1, "\"enableForwardingHttpRequest\": true"));
    try std.testing.expect(contains(u8, azure_host_json_template, 1, "\"defaultExecutablePath\": \"bootstrap\""));
    try std.testing.expect(contains(u8, azure_host_json_template, 1, "\"routePrefix\": \"\""));
    // The deploy script encodes the live-validated recipe: a Linux Consumption
    // custom-runtime function app, Oryx build disabled (else zip deploy fails with
    // "Could not detect runtime"), a catch-all anonymous HTTP function, and a
    // config-zip deploy (the preview `az functionapp deploy` API isn't available).
    inline for (.{
